/**
 * Per-route purpose/method prose for the project build-note aside. This
 * module is imported only by ProjectBuildNote, which ConditionalLayout loads
 * with next/dynamic — keep it that way so the prose ships in its own chunk
 * instead of the shared first-load bundle every route downloads.
 */
export interface ProjectBuildNoteContext {
  purpose: string;
  method: string;
}
