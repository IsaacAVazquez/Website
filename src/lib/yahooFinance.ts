 ...
 * 1. Hit fc.yahoo.com to get a Set-Cookie header
 * 2. Use that cookie to fetch the crumb from /v1/test/getcrumb
 */
async function fetchAuth(): Promise<{ crumb: string; cookie: string }> {
  // Step 1: Get cookie from Yahoo's consent/cookie endpoint
  ...
  // Build a cookie string from all Set-Cookie values
  ...
  if (!crumb || crumb.includes("<") || crumb.includes("Too Many")) {
