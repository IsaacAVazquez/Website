...
export function WeeklyBoardClient() {
  ...
  const [scoring, setScoring] = useState<FantasyRouteScoring>("ppr");
  const [board, setBoard] = useState<"flex" | "quarterbacks">("flex");
          ...
          <>
            <div className="flex flex-wrap items-center gap-3">
              <fieldset className="flex flex-wrap items-center gap-2">
                <legend className="sr-only">Scoring format</legend>
            ...
            <section aria-labelledby="weekly-waivers" className="home-card p-5">
              ...
              <p className="mt-2 max-w-[68ch] text-sm leading-6 text-[var(--home-ink-muted)]">
                ...
                him, so both numbers are printed beside it and any row can be checked by hand. A
                candidate has to be inside the top {FANTASY_WEEKLY_STARTABLE_DEPTH.flex} flex or top{" "}
                {FANTASY_WEEKLY_STARTABLE_DEPTH.quarterback} quarterbacks to count as startable, and
                rostered in under {FANTASY_WEEKLY_WIDELY_ROSTERED_PERCENT}% of leagues to count as
                available. There is no bid figure here, because the source carries none and I am not
                inventing one.
              </p>
              {waivers.length === 0 ? (
                ...
                <table className="w-full min-w-[38rem] border-collapse text-sm">
                  ...
                  <thead>
                    <tr className="border-b border-[var(--home-rule)] text-left font-mono text-3xs uppercase tracking-[0.12em] text-[var(--home-ink-muted)]">
                      ...
                      <th scope="col" className="py-2 pr-3 text-right">Expert range</th>
                      <th scope="col" className="py-2 text-right">Rostered</th>
                    </tr>
                  </thead>
                  <tbody>
                    {players.map((player) => (
