...
Update, September 7, 2026. The version described above lasted through August, and then I built the sync anyway, along with an away controller that can submit a pick in an ESPN or Sleeper tab I arm by hand. The panel now reads completed picks off the Underdog draft room and from ESPN's and Sleeper's read-only picks APIs, so the paragraph about it reading nothing and clicking nothing is no longer true, and I would rather say that here than leave it standing. What I kept from the argument is the failure story. The sync appends only a complete, uniquely matched provider log and pauses on any gap or conflict, and the controller stops for the turn whenever the page, the player, or the draft button is uncertain and lets the provider's own autopick finish it. The terms of service problem did not go away either, and the runbook in the repo says so, so this is a private tool I run on my own account with that risk understood, which is a different call than the one I made in August.
