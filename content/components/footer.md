> [!IMPORTANT]
> Historical reference only. This component reference describes an older content/template system and is not a current source of truth by itself. Use `AGENTS.md`, `CLAUDE.md`, `README.md`, `COMPONENTS.md`, and `docs/README.md` for current documentation.

# Footer Component

## Purpose
Site-wide footer providing navigation, social links, contact information, and legal/copyright details.

---

## Standard Footer Template

```markdown
---

## Isaac Vazquez

**MBA Candidate at UC Berkeley Haas**
Product & Tech Builder • Civic Tech Background

Building products that bridge technology and human impact.

---

### Quick Links

**Explore**
- [Home](/)
- [About](/about)
- [Projects](/projects)
- [Blog](/blog)
- [Contact](/contact)

**Resources**
- [Resume](/Isaac_Vazquez_Resume.pdf)
- [Newsletter](/newsletter)
- [Writing](/blog)

**Work**
- [Fantasy Football Analytics](/fantasy-football)
- [Project Portfolio](/projects)

---

### Connect

**Email:** [isaacavazquez95@gmail.com](mailto:isaacavazquez95@gmail.com)
**LinkedIn:** [linkedin.com/in/isaac-vazquez](https://www.linkedin.com/in/isaac-vazquez/)
**GitHub:** [github.com/isaacavazquez](https://github.com/isaacavazquez)
**Twitter:** [@IsaacAVazquez](https://twitter.com/IsaacAVazquez)

---

### Current Status

📚 **Learning:** Product strategy, venture capital, fintech innovation
🔨 **Building:** AI-powered civic tech tools
✅ **Open To:** Product management roles, civic tech collaborations

---

**© 2025 Isaac Vazquez. All rights reserved.**

Built with Next.js • Hosted on Netlify • [Privacy Policy](/privacy) • [Terms](/terms)

_Technology that serves humanity's best interests._
```

---

## Minimal Footer Template

```markdown
---

**Isaac Vazquez** — MBA Candidate • Product & Tech Builder

[Email](mailto:isaacavazquez95@gmail.com) • [LinkedIn](https://www.linkedin.com/in/isaac-vazquez/) • [GitHub](https://github.com/isaacavazquez) • [Resume](/Isaac_Vazquez_Resume.pdf)

© 2025 Isaac Vazquez • Built with Next.js

---
```

---

## Footer with Newsletter Signup

```markdown
---

## Stay Connected

Subscribe to get notified about new articles, projects, and insights on product management and civic tech.

[Subscribe to Newsletter →](/newsletter)

---

**Quick Links:** [About](/about) • [Projects](/projects) • [Blog](/blog) • [Contact](/contact)

**Connect:** [LinkedIn](https://www.linkedin.com/in/isaac-vazquez/) • [GitHub](https://github.com/isaacavazquez) • [Email](mailto:isaacavazquez95@gmail.com)

© 2025 Isaac Vazquez. Building technology that matters.

---
```

---

## Styling Guidelines

- **Background:** Warm Gray 800 or Warm Gray 900
- **Text Color:** Warm Gray 100 (light text on dark background)
- **Link Color:** Golden Yellow (#F7B32B) or Warm White
- **Link Hover:** Sunset Orange (#FF6B35)
- **Spacing:** Space 3XL (64px) padding top/bottom
- **Section Dividers:** Subtle line (1px, Warm Gray 700)

---

## Content Guidelines

### Essential Elements
- **Copyright:** Year and name
- **Contact:** Primary email or contact link
- **Navigation:** Key site sections
- **Social Links:** 3-5 primary social profiles

### Optional Elements
- Newsletter signup
- Current status/availability
- Site credits (built with, hosted on)
- Legal links (Privacy, Terms)
- Tagline or mission statement

---

## Best Practices

✅ **DO:**
- Keep it simple and scannable
- Include all critical links
- Use consistent spacing
- Ensure mobile responsiveness
- Update copyright year annually

❌ **DON'T:**
- Overcrowd with too many links
- Use tiny text (min 14px)
- Skip accessibility (proper contrast)
- Forget to test mobile layout
- Include broken or outdated links

---

## Accessibility Notes

- Ensure sufficient color contrast (WCAG AA minimum)
- Use semantic HTML (`<footer>` tag)
- Provide skip navigation for screen readers
- Make all links keyboard-accessible
- Use descriptive link text (not "click here")
