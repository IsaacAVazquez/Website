...
Another session held in-flight edits across the fantasy tree at critique time (both draft trackers, best ball, mock draft, `DraftValuePanel`, `useFantasySnapshot`, `fantasyUtils.ts`, `update-fantasy.yml`); `fantasy-football-client.tsx` itself was clean. Measurements ran against the dev server, meaning the working tree. Coordinate before any remediation touches those files.
