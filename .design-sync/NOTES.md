...
## Re-sync 2026-09-22: render as Catalog 97

- Isaac chose on 2026-09-22 to render the synced components as Catalog 97, since the site moved onto it on 2026-09-16 and every non-designed route now paints through the `[data-c97]` token bridge in `src/app/catalog97.css`. The bundle has no `[data-c97]` ancestor of its own, so `PreviewProvider` has to supply one (plus a `data-c97-surface` paper wrapper, because the surface rules use the descendant combinator). All previews get re-graded against that look, and the conventions header moves off the Working Instrument vocabulary.
- Result: all 32 components re-graded good on the Catalog 97 render, 180 files uploaded, `deletePaths` empty.
- The provider change is invisible to the anchor diff. `PreviewProvider` lives in the bundle, not in the preview sources, so the driver reported all 32 as unchanged and nothing pending. A deliberate re-look needs `package-capture.mjs --out ./ds-bundle --force` and a regrade of every sheet. Any future change to `preview-support.tsx`, or to the surface a component sits on, needs the same.
- Catalog 97 fonts now ship. Newsreader, Archivo, Anton, and Great Vibes woff2s (Google Fonts, OFL, pulled 2026-09-22) are declared in `.design-sync/fonts/fonts.css`, and `src/app/catalog97.css` carries in-`var()` fallbacks (`var(--font-c97-newsreader, "Newsreader")` and the rest, plus `"Fragment Mono"`), the same pattern as the Instrument fix. That retires the old `[TOKENS_MISSING]` warn. On the site next/font defines the vars, so production is unchanged.
- Known render warn, accepted 2026-09-22: `[FONT_MISSING] "Impact", "Snell Roundhand", "Petit Formal Script"`. All three are macOS or system fallbacks inside `--c97-font-numeral` and `--c97-font-script`, and the shipped Anton and Great Vibes faces sit in the same stacks, so nothing renders in an unintended font. This replaces the two 2026-08 Catalog 97 warn entries above, which assumed Catalog 97 was inert in the bundle and no longer holds.
- Preview frames must not set `borderRadius`. Three previews (ResultsTape, InstrumentTape, ClubDrawer) had inline rounded frames that read as leftover Working Instrument pills. They're zeroed now. When grepping previews for radius, search case-insensitively, since `borderRadius` has a capital R. StatusPanel's retry button now uses `c97-btn c97-btn-outline` instead of the legacy `home-button` classes.
- Site bug found and fixed during grading. `:where(.c97-page) a { color: inherit }` in `catalog97.css` was unlayered, so it beat every Tailwind `text-[...]` utility on a link and put espresso ink on oxblood `ModernButton href` fills site-wide. It now sits in `@layer base`. Isaac approved the fix on 2026-09-22.
- `Kicker` gets `cardMode: column`, because Catalog 97 letterspacing widened `PlainEyebrowHeader` past a grid cell.
- The conventions header was rewritten for Catalog 97 on Isaac's instruction. It names only `--c97-*` tokens, `.c97-*` classes, and surface values that were verified against `_ds_bundle.css` on 2026-09-22. `STYLING.md` in `guidelines/` still describes the Working Instrument, and the header says the tokens win. Revalidate after the unification close-out PR rewrites STYLING.md and deletes the bridge.
