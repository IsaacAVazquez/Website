...
tier_magnitude = clamp(overall_rank_gap / league_teams, 0, 1)
tier_signal = tier_urgency * tier_magnitude
```

A thin tier with no meaningful gap on the overall board therefore stays small. A final published tier has no next tier to measure and is reported as unavailable instead of being treated as the largest cliff on the board. The magnitude divides by the selected league size because the yardstick is one round of picks in the room actually being drafted, so the same gap reads steeper in an 8-team room than a 16-team one. Until 2026-08-31 this divided by a fixed twelve in every room.

The wait reading appears only while the user is on the clock and compares the present pick with the user's following pick, respecting either snake or linear order. It looks through the available players at one position and finds the best player by consensus rank whose current ADP midpoint is at or after that following pick. The panel reports the consensus rank cost and the drop in replacement index from the best option now, and when the FantasyPros VORP source covers the league size and remains usable it also prices the same drop in projected season points, which is the difference between the two players' points above replacement. Rooms outside the covered sizes and rooms with a stale VORP source keep the ordinal reading alone. ADP decides who the market prices to remain available, while consensus rank and the fixed replacement lines decide the size of the drop.

A player needs at least 20 observed selections before his ADP can support this reading. The survival band around a player's ADP comes from the published ADP spread alone, floored at six picks, because expert disagreement is about how good a player is rather than when rooms draft him. The value and reach labels elsewhere keep the wider combined band, since a rank-versus-ADP gap is noisy from both sources. `ADP + band` identifies a plausible survivor inside the later side of the market range, while `ADP - band` identifies the safer survivor whose range still clears the following pick. If no midpoint survivor exists but a plausible survivor does, the panel names that distinction. Missing reliable prices ahead of the reported survivor reduce coverage to limited. Until 2026-08-31 the survival band reused the combined value threshold, which overstated how long contested players last.
...
The snapshot build keeps a small committed history of each player's overall consensus rank and ADP, one dated reading per run per scoring format, trimmed to about two weeks (`src/data/fantasyRankHistory.generated.json`). At build time it stamps 7 and 14-day movement onto the overall and flex boards as past value minus current value, so a positive figure means the player moved up the board. Movement appears only once the history actually holds a reading old enough for the window, there is no interpolation, and the position boards stay out because they rank on their own scale. The player drawer shows the figures when they exist.

## Recommendation telemetry and recap

The redraft tracker records what the decision strip showed at each of the user's turns, on that device only, keyed to the room's draft id. A record stores the recommended players, the best available player, the priced position and its baseline, the expected market-midpoint survivor, the predicted wait cost in spots and points, the most-at-risk player, the model version and source dates, and the available position board with its ranks and VORP values. Outcomes are never stored. The post-draft recap derives them from the final pick log and those frozen draft-time inputs, so an undone and replayed turn cannot leave a stale verdict, a record whose pick no longer matches the log is ignored, and a daily snapshot refresh cannot change a completed recap. The recap reports how often the pick followed a recommendation, how often the expected survivor actually lasted, how often the flagged player was gone by the next turn, and the predicted versus realized drop at the priced position. The drafter's own pick at the recorded turn does not count toward market attrition. The recap averages the predicted points cost over the same turns as the realized one, so the two columns calibrate against each other, and an unfinished room can open the same recap from a disclosure below the board. What I want from it is evidence for changing the model's weights rather than a grade of the drafter.
...
npm test -- --runInBand \
  ...
  src/lib/__tests__/fantasyUtils.test.ts \
  src/lib/__tests__/fantasyRankHistory.test.ts \
  src/lib/__tests__/redraftLineup.test.ts \
  src/lib/__tests__/draftAnalytics.test.ts \
  src/lib/__tests__/draftTelemetry.test.ts \
  src/lib/__tests__/draftPresets.test.ts \
  src/lib/__tests__/fantasyTeamValue.test.ts \
  src/lib/__tests__/fantasyTrade.test.ts \
  src/lib/__tests__/fantasyTradePersistence.test.ts \
  src/lib/__tests__/mockDraft.test.ts \
